-- Approved NGOs can publish missions; point constants stay centrally configurable.
...
begin
  ...
  select * into rules from public.point_rules where point_rules.id = true;
  ...
  return query
  select saved_mission_id, trim(p_slug),
    round(base_points * multiplier)::integer;
end;
$$;
