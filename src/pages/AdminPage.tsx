...
export function AdminPage() {
  ...
  if (!isAdmin) return <main className="mx-auto grid min-h-dvh max-w-md place-items-center bg-white p-6 text-center"><div><ShieldCheck className="mx-auto text-sky-500" size={34} /><h1 className="mt-4 text-2xl font-bold">Accès administrateur</h1><p className="mt-2 text-sm text-slate-500">{user ? 'Ce compte ne possède pas les droits administrateur.' : 'Ce tableau de bord est réservé à l’équipe dfi3a.'}</p>{!user && <button className="mt-6 min-h-12 rounded-full bg-sky-500 px-6 text-sm font-bold text-white" onClick={() => navigate('/auth?mode=login&returnTo=%2Fadmin')} type="button">Se connecter</button>}<button className={`${user ? 'mt-6' : 'mt-3'} min-h-11 w-full text-sm font-semibold text-slate-500`} onClick={() => navigate('/')} type="button">Retour à l’accueil</button></div></main>
