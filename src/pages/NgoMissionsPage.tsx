import { Ban, CalendarDays, MapPin, Plus, UsersRound } from 'lucide-react'
  ...
  return (
    <div className="pt-[max(0.5rem,env(safe-area-inset-top))]">
      ...
      {isLoading ? (
        ...
        <section className="mt-5 space-y-3">{visibleMissions.map((mission) => <MissionCard key={mission.id} mission={mission} onCancel={() => { setSuccessMessage(''); setMissionToCancel(mission) }} />)}</section>
      )}

      {missionToCancel && (
        <CancelMissionSheet
          missionId={missionToCancel.id}
          missionTitle={missionToCancel.title}
          onCancelled={(count) => {
            setMissions((current) => current.map((mission) => mission.id === missionToCancel.id ? { ...mission, status: 'cancelled' } : mission))
...
function MissionCard({ mission, onCancel }: { mission: Mission; onCancel: () => void }) {
  ...
  return (
    <article className="rounded-[22px] bg-white p-4 shadow-sm">
      ...
      <p className="mt-2 text-sm font-bold text-sky-600"><DBuxAmount amount={mission.total_points} /></p>
