  ...
  return (
    <div className="pt-[max(0.5rem,env(safe-area-inset-top))]">
      ...
      {missions.length === 0 ? (
        ...
        <section className="mt-4 space-y-3">{missions.map((mission) => <article className="rounded-[20px] bg-white p-4 shadow-sm" key={mission.id}><div className="flex items-start justify-between gap-3"><div><h3 className="font-bold">{mission.title}</h3><p className="mt-1 text-xs text-slate-500">{new Intl.DateTimeFormat('fr-FR', { dateStyle: 'medium', timeStyle: 'short', timeZone: MOROCCO_TIME_ZONE }).format(new Date(mission.starts_at))}</p></div><span className={`rounded-full px-2 py-1 text-[10px] font-bold ${mission.urgency_status === 'approved' ? 'bg-rose-50 text-rose-600' : mission.urgency_status === 'pending' ? 'bg-amber-50 text-amber-700' : 'bg-slate-100 text-slate-600'}`}>{mission.urgency_status === 'approved' ? 'Urgente' : mission.urgency_status === 'pending' ? 'Urgence en attente' : missionStatusLabels[mission.status] ?? mission.status}</span></div><p className="mt-3 text-xs font-semibold text-sky-600"><DBuxAmount amount={mission.total_points} iconClassName="h-3.5 w-auto" /></p><div className="mt-3 flex gap-2 border-t border-slate-100 pt-3"><Link className="flex-1 text-center text-xs font-bold text-slate-600" to={`/missions/${mission.slug}`}>Voir la mission</Link><Link className="flex-1 text-center text-xs font-bold text-sky-700" to={`/ngo/missions/${mission.id}/attendance`}>Gérer les présences</Link></div></article>)}</section>
      )}
    </div>
  )
}
